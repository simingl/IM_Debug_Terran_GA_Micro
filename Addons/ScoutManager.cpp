...
void ScoutManager::setBaseManager(BaseManager* baseManager)
{
...
void ScoutManager::scanEnemyBase()
{
	if (scanBaseCount >= baseLocationsToScanVector.size())
		scanBaseCount = 0;

	BWTA::BaseLocation* base = baseLocationsToScanVector[this->scanBaseCount];

	UnitGroup comsats = SelectAll(BWAPI::UnitTypes::Terran_Comsat_Station)(isCompleted);
	for each(BWAPI::Unit *c in comsats)
	{
		if(c->getEnergy() >= 150 )
		{
			c->useTech(BWAPI::TechTypes::Scanner_Sweep, base->getPosition());
			this->scanBaseCount++;
			break;
		}
	}
...
void ScoutManager::updateBasesNotOwn()
{
	std::set<Base*> myBases = this->baseManager->getAllBases();
	std::set<BWTA::BaseLocation*> allBases = BWTA::getBaseLocations();

	for (std::set<Base*>::iterator i=myBases.begin(); i != myBases.end(); i++)
	{
		BWTA::BaseLocation* base = (*i)->getBaseLocation();
		if (allBases.find(base) != allBases.end())
		{
			allBases.erase(base);
		}
	}
	
	this->baseLocationsToScan = allBases;	

	baseLocationsToScanVector.clear();
	for(std::set<BWTA::BaseLocation*>::iterator i=baseLocationsToScan.begin();i!=baseLocationsToScan.end();i++)
		baseLocationsToScanVector.push_back(*i);
}