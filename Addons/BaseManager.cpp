...
BWTA::BaseLocation* BaseManager::getNextExpandLocation()
{
	BWTA::BaseLocation* location=BWTA::getStartLocation(BWAPI::Broodwar->self());
	double minDist=-1;
...
void BaseManager::updateAddons()
{
	std::set<Base*> activeBases = this->getActiveBases();

	for(std::set<Base*>::iterator b = activeBases.begin(); b != activeBases.end(); b++)
	{
		if ((*b)->getResourceDepot()->getAddon() == NULL)
		{
			(*b)->getResourceDepot()->buildAddon(BWAPI::UnitTypes::Terran_Comsat_Station);
		}
	}
}
