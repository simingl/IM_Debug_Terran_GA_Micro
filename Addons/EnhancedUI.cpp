...
void EnhancedUI::update() const
{
  ...
  //drawProgress();
	...
	for(set<BWAPI::Unit*>::iterator i=Broodwar->getAllUnits().begin();i!=Broodwar->getAllUnits().end();i++) {
		drawUnitHealth(*i);
	}
}
...
void EnhancedUI::drawTerrain() const
{
  ...
  for(std::set<BWTA::Polygon*>::const_iterator r=BWTA::getUnwalkablePolygons().begin();r!=BWTA::getUnwalkablePolygons().end();r++)
  {
    ...
    for(int j=0;j<(int)p->size();j++)
    {
      Position point1=(*p)[j] ;
      Position point2=(*p)[(j+1) % p->size()] ;
	  Broodwar->drawLine(CoordinateType::Map,point1.x()*8,point1.y()*8,point2.x()*8,point2.y()*8,Colors::Red);
    }
  }
...
void EnhancedUI::drawUnitHealth(Unit* unit) const
{
	// Hit points
	if (!unit->isVisible())
		return;
